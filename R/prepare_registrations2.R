...
#' on scores to the data on registrations. Creates two versions of scores: a max score as an applicant
#' and a max score as an acepted applicant. This might needed in the case of multiple paths
#' in the admissions process (e.g. an extra path for transfer students).
...
prepare_registrations <- function(registrations = NULL, scores = NULL,
  ...
  dataOnRegistrations <- registrations %>%
    mutate(wynik = ifelse(inna_punktacja == 0,wynik,NA), # removes the scores if there is a special scoring scheme for some registration codes.
           wynik_zak = ifelse(zakwalifikowany %in% "1",wynik,NA)) %>% # removes the scores if someone was not accepted
    ...
    summarise(
      ...
      pkt_kan = suppressWarnings(
        max(wynik[!is.na(wynik)])), # the highest achieved score
      pkt_zak = suppressWarnings(
        max(wynik[!is.na(wynik_zak)])) # the highest achieved score when accepted
    ) %>%
    mutate(pkt_kan = ifelse(pkt_kan == -Inf, NA, pkt_kan),
           pkt_zak = ifelse(pkt_zak == -Inf, NA, pkt_zak)) %>%
    ungroup()
