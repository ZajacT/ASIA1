#' @title Preparing recrutation data for analysis - a new test version
...
#' @examples
#' \dontrun{
#' prepare_registrations3("inst/pr_zapisy.xlsx", "inst/pr_wyniki.xlsx",
#'                       "dane/dane-IRK poprawione.csv", "inst/pr_usos.xlsx",
#'                       "inst/pr_rek_irk.xlsx", "inst/pr_rek_usos.xlsx")
#' }
#' @export
prepare_registrations3 <- function(registrations = NULL, scores = NULL,
  ...
  dataOnRegistrations <- registrations %>%
    ...
    ungroup() %>%
    group_by(pesel, studia) %>% # computing the eligibility of each application for admitssion 
