...
#' @description Function checks if data on registretions are complete and
...
check_registrations <- function(registrations) {
  ...
  #-----------------------------------------------------------------------------
  reqVarNames <- c("pesel", "studia", "czy_oplacony", "zakwalifikowany", "przyjety")
  missVarNames  <- setdiff(reqVarNames, names(registrations))
  if (length(missVarNames) > 0) {
    stop(paste0("W danych o rekrutacjach brakuje ",
                ifelse(length(missVarNames) > 1, "zmiennych", "zmiennej"), ": '",
                paste0(missVarNames, collapse = "', '"),
                "'."), call. = FALSE)
  }
  #-----------------------------------------------------------------------------
  #|-> checks whether variables contains any illegal values and offers fixes
  #-----------------------------------------------------------------------------
