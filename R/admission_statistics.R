...
#' @importFrom utils write.table menu
#' @export
admission_statistics <- function(groupingVariable = "studia", registrations = NULL, exams = NULL,
  ...
  #-----------------------------------------------------------------------------
  #|-> Decision whether data should be presented if the number of observations is small
  #----------------------------------------------------------------------------- 
  
  switch(menu(c("Wyliczone mają być wszystkie statystyki",
                "Wyliczone mają być wybrane statystyki dla odpowiednio licznych grup na potrzeby strony ASIA"),
              title = "Jaki zestaw statystyk ma być przygotowany"),
         n_check <- 1,
         n_check <- 2)
  ...
  if(n_check == 2){
    registrations <- registrations %>%
      ...
      mutate( n_prz_pkt = sum(prz %in% 1 & !is.na(pkt)),
        pkt = ifelse(n_prz_pkt > 9, pkt, NA)
      ) %>%
      ungroup()
  }
  ...
  #-----------------------------------------------------------------------------
  #|-> Selecting data for ASIA website
  #-----------------------------------------------------------------------------
